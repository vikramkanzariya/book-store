    ...
    return (
        <Grid item xs={12} sm={4} key={cartItem.id}>
            <Item sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', flexDirection: 'column', width: '100%', boxShadow: '2' }}>
                <Typography sx={{ backgroundColor: '#7688dd', marginBottom: '5px', padding: '10px', fontSize: '1.2rem', width: '100%', fontWeight: '700', color: 'white', fontVariant: 'petite-caps' }}>{cartItem.book.name}</Typography>
                ...
                <Box sx={{ backgroundColor: '#7688dd', margin: '10px', width: '100%', padding: '10px' }}>
                ...
                /> */}
                    <Button id={`${cartItem.id}-cartButton`} sx={{ marginTop: '10px', color: "white", borderColor: "white" }} variant='outlined'>Remove</Button>
                </Box>
            </Item>
        </Grid>
    )
