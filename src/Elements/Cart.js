...
const Cart = () => {
  ...
  return (
    <Container sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', flexGrow: 1, marginTop: '30px', marginBottom: '50px', marginLeft: '10px', marginRight: '10px', width: '100%' }}>
      <Typography variant='h2' sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', width: '100%', backgroundColor: theme.palette.primary.light, textAlign: 'center', margin: '10px', borderRadius: '10px' }}>
        Cart
      </Typography>
      ...
      <Button onClick={() => { placeOrder(); toast("Order Placed"); }} sx={{ display: cart.length === 0 ? 'none' : 'block', width: '150px', marginTop: '20px', backgroundColor: theme.palette.primary.light, "&:hover": { backgroundColor: theme.palette.primary.dark } }} variant='contained'>Place Order</Button>
    </Container>
  )
}
